...
void tim011_state::tim011_io(address_map &map)
{
	...
	map(0x8000, 0xffff).ram().share("videoram"); // Video RAM 43256 SRAM  (32KB)
}
...
uint32_t tim011_state::screen_update_tim011(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	for (u8 x = 0; x < 512/4; x++)
	{
		for (u16 y = 0; y < 256; y++)
		{
			int horpos = x*4;
			u8 code = m_vram[addr++];
			u8 realy = (y - m_scroll) & 0xff;
			bitmap.pix(realy, horpos++) =  (code >> 0) & 0x03;
			bitmap.pix(realy, horpos++) =  (code >> 2) & 0x03;
			bitmap.pix(realy, horpos++) =  (code >> 4) & 0x03;
			bitmap.pix(realy, horpos++) =  (code >> 6) & 0x03;
		}
	}
	return 0;
}
...
ROM_START( tim011 )
	...
	ROM_LOAD( "keyb_tim011.ic4", 0x0000, 0x1000, CRC(a99c40a6) SHA1(d6d505271d91df4e079ec3c0a4abbe75ae9d649b))
ROM_END
